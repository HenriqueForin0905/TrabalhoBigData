"""
Gerar tabelas Gold (KPIs) a partir da camada Silver.

Exemplos de KPIs:
- preço médio por categoria
- variação de preço ao longo do tempo (mês)
...
def avg_price_by_category(products_df: pd.DataFrame, items_df: pd.DataFrame) -> pd.DataFrame:
    # products_df should contain product_id and category
    # items_df should contain product_id and price
...
def price_variation_over_time(items_df: pd.DataFrame) -> pd.DataFrame:
    # expects order_approved_at or similar datetime
    df = items_df.copy()
